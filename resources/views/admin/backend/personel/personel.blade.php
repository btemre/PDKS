                        ...
                        <div class="card-body">
                            <div class="row g-4 mb-3">
                                <div class="col-sm-auto">
                                    ...
                                    <a class="btn btn-success" onclick="add()" href="javascript:void(0)">Personel Ekle</a>
                                    @endif
                                    <!-- <a class="btn btn-primary" href="{{ route('personel.ayrilan') }}">Ayrılış Yapan
                                        Personel</a>
                                        <a class="btn btn-soft-warning waves-effect waves-light shadow-none" target="_blank" href="{{ route('personel.memurkapak') }}">Memur Dosya Kapağı</a>
                                        <a class="btn btn-soft-info waves-effect waves-light shadow-none" target="_blank" href="{{ route('personel.iscikapak') }}">İşçi Dosya Kapağı</a>
                                        <a class="btn btn-soft-success waves-effect waves-light shadow-none" target="_blank" href="{{ route('personel.firmakapak') }}">Firma Dosya Kapağı</a> -->
                                </div>
                            </div>
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-lg-2">
                                ...
                                <select id="personel_il" class="form-control" name="personel_il">
                                    <option value="">Seçiniz</option>
                                    @foreach ($il as $value)
                                        <option value="{{ $value->il_plaka }}">{{ $value->il_ad }}</option>
                                    @endforeach
                                </select>
                            </div>
    ...
    <script type="text/javascript">
        ...
        function add() {
            ...
            $('#company-logo-input').val(''); // file inputu sıfırla
                // İl ve ilçe selectlerini resetle
    $('#personel_il').val('');
    $('#personel_ilce').html('<option value="">Seçiniz</option>');
            ...
            $.ajax({
                ...
                success: function(res) {
                    if (res.status === 'success') {
                        ...
                        $('#personel_kartkullanim').val(res.data.personel_kartkullanim);
...
// İlçe selectini AJAX ile yükle ve mevcut ilçe seç
var ilce_id = res.data.personel_ilce;
if(res.data.personel_il) {
    $.get('/get-ilce/' + res.data.personel_il, function(data) {
        $('#personel_ilce').html('<option value="">Seçiniz</option>');
        $.each(data, function(key, value) {
            $('#personel_ilce').append('<option value="'+ value.ilce_id +'">'+ value.ilce_ad +'</option>');
        });
        $('#personel_ilce').val(ilce_id); // mevcut ilçe seçili
    });
} else {
    $('#personel_ilce').html('<option value="">Seçiniz</option>');
}
        ...
        function deleteFunc(personel_id) {
            Swal.fire({
        ...
        $('#personel_il').on('change', function() {
    var il_plaka = $(this).val();
    $('#personel_ilce').html('<option value="">Seçiniz</option>'); // reset

    if(il_plaka) {
        $.get('/get-ilce/' + il_plaka, function(data) {
            $.each(data, function(key, value) {
                $('#personel_ilce').append('<option value="'+ value.ilce_id +'">'+ value.ilce_ad +'</option>');
            });
        });
